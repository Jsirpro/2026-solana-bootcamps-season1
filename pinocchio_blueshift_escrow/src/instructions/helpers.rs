...
impl ProgramAccount {
    ...
    /// 关闭 PDA
    pub fn close(
        account: &AccountView,      // 要关闭的 PDA（你的程序拥有）
        destination: &AccountView,  // 接收 lamports 的账户（通常是用户）
    ) -> ProgramResult {
        log!("[close] start");
        log!("helpers->close()->unsafe(),lamports of escrow:{}",account.lamports());
        log!("helpers->close()->unsafe(),lamports of maker:{}",destination.lamports());
        
        destination.set_lamports(account.lamports() + destination.lamports());
